...
func _start_intro_dialogue() -> void:
	if dialogue_ui:
		dialogue_ui.show_dialogue([
			"Each lectern will spawn different amount of potions.",
			"It is important that you find the lectern first."
		])
...
func _process(_delta: float) -> void:
	...
	if current_state == LevelState.EXPLORING and not dialogue_ui.is_active():
		if _player_near_lectern():
			...
			dialogue_ui.show_dialogue([
				"Do you see the lectern glowing? Red is when you need to combine potencies,",
				"Blue for reducing, Yellow for multiplying, Green for division, and Pink for ratios."
			])
